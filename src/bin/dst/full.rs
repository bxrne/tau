...
pub(crate) struct CellResult {
    ...
    pub(crate) fault_injection: &'static str,
...
pub(crate) fn run_full(seed: u64, cli: &Cli) {
    ...
    if !tau_bin.exists() {
        error!("tau binary not found at {tau_bin:?}. Build with: cargo build --release");
        process::exit(1);
    }
...
fn setup_cell(
    ...
    seed: u64,
) -> Result<(Server, Conn), CellResult> {
    let server =
        Server::spawn(cell, scratch_path).map_err(|_| cell_error(cell, seed, "SPAWN_ERR"))?;
    server
        .wait_ready()
...
fn run_ops(
    ...
    for op_idx in 0..cli.ops {
        ...
        if let Some(err) = execute_append_and_verify(
            conn,
            oracle,
            rng,
            &mut time_cursor,
...
fn run_cell(cell: &Cell, cli: &Cli, scratch_root: &Path, seed: u64) -> CellResult {
    ...
    let (server, mut conn) = match setup_cell(cell, scratch.as_ref().map(|s| s.path()), seed) {
        Ok(v) => v,
        Err(result) => return result,
    };

    let t0 = Instant::now();
    let (correctness, fault_result, successful_ops, append_count) = run_ops(
        &mut conn,
        &mut oracle,
        &mut rng,
        &server,
