...
```bash
# Start a local cluster (skip if you already have one)
minikube start

# Build and load the image into minikube
docker build -f container/Dockerfile -t tau:local .
minikube image load tau:local

# Install from the local chart
helm install tau container/helm/tau \
  --set image.repository=tau \
  --set image.tag=local \
  --set image.pullPolicy=Never \
  --set service.type=LoadBalancer

# Get access URL (minikube)
minikube service tau --url

# Connect
cargo run --release --bin ctl
# τ connect local <host>:7070
```

The published chart is available at `oci://ghcr.io/bxrne/charts/tau` for each release tag. See [Containers](docs/content/docs/containers.md) for the full values reference.
