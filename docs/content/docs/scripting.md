...
2. **DST** (`crates/dst/`) — canned functions with known semantics are dual-simulated: the target runs the Lua, the oracle computes the expected effect independently. See [DST](/docs/dst/) and `crates/dst/LUA_TESTING.md`.
