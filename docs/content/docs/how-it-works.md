...
Tau is structured as a library (`libtau`) consumed by three binaries: the TCP server (`tau`), the interactive client (`ctl`, ratatui TUI with `--headless` fallback). The library exposes a clean `Executor` API; auth, TLS, and network concerns live exclusively in the server.
