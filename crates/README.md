...
| [`bench`](bench/README.md) | library + binary (`benchtau`) | Deterministic workload generator and runner: drives `libtau::Executor` directly and a live `tau` server over the wire, across a config grid. |

Binary names are unique across the workspace, so `cargo run --bin tau|tauctl|benchtau` works from the repo root.
