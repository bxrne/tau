...
mod tests {
    ...
    #[hegel::test]
    fn lens_base_eval_always_none(tc: TestCase) {
        let probe = tc.draw(gs::integers::<i64>());
        let lens: Lens<i32> = Lens {
            name: "x".into(),
            kind: LensKind::Base,
        };
        assert_eq!(lens.eval(probe), None);
    }

    #[hegel::test]
    fn lens_derived_eval_delegates_to_closure(tc: TestCase) {
        let probe = tc.draw(gs::integers::<i64>().min_value(-1000).max_value(1000));
        let f: Eval<i64> = Arc::new(|t| Some(t * 2));
        let lens = Lens {
            name: "x".into(),
            kind: LensKind::Derived(f),
        };
        assert_eq!(lens.eval(probe), Some(probe * 2));
    }
    ...
    fn lens_kind_derived_clone_shares_arc() {
        ...
        let LensKind::Derived(g) = kind.clone() else {
