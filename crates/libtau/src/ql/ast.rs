...
//! range  := RANGE  LENS <ident> <int> <int> [WHERE <expr>] [LIMIT <uint> [OFFSET <uint>]]
...
pub enum Stmt {
    ...
    /// `SET TTL LENS <name> <secs>` — expire data whose temporal interval ends
    /// more than `secs` seconds before the current query time.  Persisted in
    /// the schema WAL so it survives a restart.
    SetTtl {
        name: String,
        secs: i64,
    },
    /// `UNSET TTL LENS <name>` — remove the TTL policy for a lens, making all
    /// its data permanently visible again.  Persisted in the schema WAL.
    UnsetTtl {
        name: String,
    },
