...
pub enum StorageBackend {
    ...
    Disk {
        ...
        /// When `false`, the disk file is not rewritten after each compaction.
...
impl DbState {
    ...
    fn with_disk(
        ...
        store.set_rewrite_on_compact(rewrite_on_compact);
...
fn stmt_to_op(stmt: &Stmt) -> Op {
    match stmt {
        ...
        Stmt::ShowDatabases | Stmt::ShowLenses | Stmt::ShowUsers | Stmt::ShowGrants { .. } => {
            Op::Show
        }
...
impl Executor {
    ...
    /// Toggle whether disk stores rewrite their file after each compaction.
    /// Only affects the disk backend; no-op on memory-backed executors.
    /// Must be called before `CREATE DATABASE` to take effect on new databases.
    pub fn set_disk_rewrite_on_compact(&mut self, on: bool) {
        if let StorageBackend::Disk {
            ref mut rewrite_on_compact,
            ..
        } = self.backend
        {
            *rewrite_on_compact = on;
        }
    }
    ...
    pub fn exec_read(&self, stmt: &Stmt) -> Result<Output, ExecError> {
        ...
        if let Some(db) = self.active.as_deref() {
